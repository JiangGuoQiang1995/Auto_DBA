...
namespace Microsoft.VsSDK.UnitTestLibrary
{
    ...
    public class LocalRegistryMock : ILocalRegistry, ILocalRegistry2, ILocalRegistry3
    {
        ...
        {
            this.objectsList = new Dictionary<Guid, object>();
        }
        public void AddClass(Type classType, object instance)
        {
            if (this.objectsList.ContainsKey(classType.GUID))
            {
                this.objectsList.Remove(classType.GUID);
            }
            this.objectsList.Add(classType.GUID, instance);
        }
        public string RegistryRoot
        {
            get { return this.registryRoot; }
            set { this.registryRoot = value; }
        }

        public int CreateInstance(Guid clsid, object punkOuter, ref Guid riid, uint dwFlags, out IntPtr ppvObj)
        {
            ppvObj = IntPtr.Zero;
            if (!this.objectsList.ContainsKey(clsid))
            {
                return Microsoft.VisualStudio.VSConstants.E_NOINTERFACE;
            }
            var obj = this.objectsList[clsid];
        ...
        [SuppressMessage("Microsoft.Globalization", "CA1303:DoNotPassLiteralsAsLocalizedParameters")]
        public virtual int GetTypeLibOfClsid(Guid clsid, out ITypeLib pptLib)
        {
            throw new NotImplementedException("The method or operation is not implemented.");
        }
    }
