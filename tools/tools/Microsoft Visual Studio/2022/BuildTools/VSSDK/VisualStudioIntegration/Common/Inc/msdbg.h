...
enum AD7_HRESULT
{
    ...
    // If the frame is a proxy frame representing one or more other tasks.
    S_PROXY_FRAME__ = 0x40D47,
    // Informs the Visualizer CustomViewer that a modal dialog was opened so that it does not force the EE windows to refresh after the visualizer's DisplayValue method returns.
    S_OPENED_NON_MODAL__ = 0x40D4A,
    ...
    // Failed to initialize msdbg2.dll for script debugging. If this problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Visual Studio Remote Debugger (MSVSMON.EXE) failed to start. If this problem persists, please repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Unable to load Visual Studio debugger component (vsdebugeng.dll). If this problem persists, repair your Visual Studio installation from the Visual Studio Installer
    E_LOAD_VSDEBUGENG_FAILED__ = 0x80040C10,
    // Unable to initialize Visual Studio debugger component (vsdebugeng.dll). If this problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    E_LOAD_VSDEBUGENG_IMPORTS_FAILED__ = 0x80040C11,
    // Unable to initialize Visual Studio debugger due to a configuration error. If this problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Unable to load a Visual Studio component (VSDebugScriptAgent110.dll). If the problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Unable to find a Visual Studio component (VsDebugLaunchNotify.exe). For remote debugging, this file must be present on the target computer. If the problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Unable to load a Visual Studio component (symbollocator.resources.dll). If the problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    // Unable to load a Visual Studio component (MSEnc.resources.dll). If the problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    ...
    E_PRELAUNCH_TARGET_NOT_SUPPORTED__ = 0x80040D1D,
    // Unable to load Visual Studio debugger component (msvcdis140.dll). If this problem persists, repair your Visual Studio installation from the Visual Studio Installer.
    E_MSVCDIS_LOAD_FAIL__ = 0x80040D1E,
    // Mixed-mode debugging is not supported with the legacy managed debug engine.
    E_LEGACY_INTEROP_NOT_SUPPORTED__ = 0x80040D1F,
    ...
    E_CANNOT_DEBUG_X86__ = 0x80040D20,
    ...
    E_CANNOT_DEBUG_AMD64__ = 0x80040D21,
    ...
    E_CANNOT_DEBUG_ARM__ = 0x80040D22,
    ...
    E_CANNOT_DEBUG_ARM64__ = 0x80040D23,
    // The call to attach a debug program to the debug engine was received in an unexpected order.
    E_INVALID_ATTACH_ORDER__ = 0x80040D24,
    // The async operation is already running.
    E_ASYNC_OPERATION_RUNNING__ = 0x80040D25,
    // The async operation is not running.
    E_ASYNC_OPERATION_NOT_RUNNING__ = 0x80040D26,
    // The async operation has been added to a worklist and should not be managed manually.
    E_OPERATION_IN_WORKLIST__ = 0x80040D27,
    // Failed to post an async operation completion to the UI thread.
    E_FAILED_TO_POST_COMPLETION__ = 0x80040D28,
    // Cannot begin an async operation while the SDM is cancelling current operations.
    E_CANCELLING_OPERATIONS__ = 0x80040D29,
    // This Time Travel Debugging trace file is already being debugged.
    E_TTD_TRACE_ALREADY_LAUNCHED__ = 0x80040D2A,
    // Script debugging is not supported on the target platform.
    E_SCRIPT_NOT_SUPPORTED__ = 0x80040D2B,
    // Tasks cannot be shown if a dump has multiple app domains.
    E_DUMP_WITH_MULTIPLE_APPDOMAINS__ = 0x80040D2C,
    // The thread is virtual.
    E_VIRTUAL_THREAD__ = 0x80040D2D,
    // Viewing tasks for live processes is not supported in .NET Core 2.2.
    E_LIVE_TASKS_NOT_SUPPORTED_NETCORE_2_2__ = 0x80040D2E,
    // Viewing tasks for live processes is not supported in attach scenario.
    E_LIVE_TASKS_NOT_SUPPORTED_ATTACH__ = 0x80040D2F,
    // The CLR version is not supported to retrieve task information for the dump.
    E_DUMP_TASKS_NOT_SUPPORTED_CLR_VERSION__ = 0x80040D30,
    // Viewing Task information for live processes while debugging is not supported on Windows versions before 8.1.
    E_LIVE_TASKS_NOT_SUPPORTED_BEFORE_WINDOWS_8_1__ = 0x80040D31,
    // The Docker command failed. See Output Window for more details.
    E_DOCKER_COMMAND_ERROR__ = 0x80040D32,
    // Unable to launch the Visual Studio Remote Debugger (MSVSMON.EXE) in the Docker Windows container. See Output Window for more details.
    E_DOCKER_MSVSMON_LAUNCH_FAILED__ = 0x80040D33,
    // Unable to confirm that the Docker container is running.
    E_DOCKER_CONTAINER_NOT_RUNNING__ = 0x80040D34,
    // File or directory not found in Docker container.
    E_DOCKER_MISSING_FILEORDIRECTORY__ = 0x80040D36,
    // Command took longer than expected to complete.
    E_DOCKER_COMMAND_RAN_LONGER__ = 0x80040D37,
    // Failed to run 'docker.exe'. Verify the Docker CLI is installed and can be located in PATH.
    E_DOCKER_NOT_FOUND__ = 0x80040D38,
    // Invalid Docker container architecture. Expected architectures are 'X86', 'AMD64', 'ARM' and 'ARM64'.
    E_DOCKER_INVALIDARCHITECTURE__ = 0x80040D39,
    // Unable to download or extract debugger. See Output Window for more details.
    E_DOCKER_UNABLETODOWNLOAD__ = 0x80040D3A,
    // Unexpected output from command.
    E_DOCKER_UNEXPECTEDOUTPUT__ = 0x80040D3B,
    // Docker (Windows Container) feature not registered properly.
    E_DOCKER_NOT_REGISTERED_PROPERLY__ = 0x80040D3C,
    // Docker command execution canceled by user.
    E_DOCKER_USERCANCELED__ = 0x80040D3D,
    // Required command line tool missing in Docker Container. See Output Window for more details.
    E_DOCKER_MISSINGREQUIREDTOOL__ = 0x80040D3F,
    // Unsupported operation. Docker Windows container attach is only available for target processes that have the same processor architecture as the Docker container.
    E_DOCKER_MATCHINGARCHSUPPORT__ = 0x80040D40,
    // Decompiled symbols must be portable PDB Format
    E_DECOMPILED_SYMBOLS_MUST_BE_PORTABLE_PDB_FORMAT__ = 0x80040D41,
    // Modules that have been edited using Edit-In-Continue cannot be decompiled.
    E_DECOMPILED_SYMBOLS_NO_EDIT_IN_CONTINUE__ = 0x80040D42,
    // Modules that have the SuppressIldasmAttribute cannot be decompiled.
    E_DECOMPILED_SYMBOLS_SUPRESS_ILDASM__ = 0x80040D43,
    // Unable to remap the instruction pointer to the updated method.
    E_ENC_CANNOT_REMAP_AFTER_SET_NEXT_STATEMENT__ = 0x80040D44,
    // The binary for mscorlib/System.Private.CoreLib has not been loaded.
    E_BCL_METADATA_MISSING__ = 0x80040D45,
    // Unable to view task information because ETW resources have been exhausted. This may be caused by debugging multiple processes on the same computer.
    E_LIVE_TASKS_NOT_SUPPORTED_TOO_MANY_SESSIONS__ = 0x80040D46,
    // The memory layout of the module is not supported.
    E_DECOMPILED_UNSUPPORTED_MEMORY_LAYOUT__ = 0x80040D48,
    // The Profiler was unable to launch the current startup project.
    E_PROFILER_LAUNCH_FAILURE__ = 0x80040D49,
    // The debugger does not support debugging managed and native code at the same time in an emulated (x64 on ARM64) process. Configure the debugger to debug only native code or only managed code.
    E_EMULATED_INTEROP_NOT_SUPPORTED__ = 0x80040D4B,
    // The debugger has located a executable, symbol, or source file, but it is in an untrusted location.
    E_UNTRUSTED_LOCATION__ = 0x80040D4C,
    // Operation is not allowed in no-debug launch scenarios
    E_NO_DEBUG_LAUNCH__ = 0x80040D4D
};
...
#define S_PROXY_FRAME _HRESULT_TYPEDEF_(S_PROXY_FRAME__)
#define S_OPENED_NON_MODAL _HRESULT_TYPEDEF_(S_OPENED_NON_MODAL__)
...
#define E_PRELAUNCH_TARGET_NOT_SUPPORTED _HRESULT_TYPEDEF_(E_PRELAUNCH_TARGET_NOT_SUPPORTED__)
#define E_MSVCDIS_LOAD_FAIL _HRESULT_TYPEDEF_(E_MSVCDIS_LOAD_FAIL__)
#define E_LEGACY_INTEROP_NOT_SUPPORTED _HRESULT_TYPEDEF_(E_LEGACY_INTEROP_NOT_SUPPORTED__)
#define E_CANNOT_DEBUG_X86 _HRESULT_TYPEDEF_(E_CANNOT_DEBUG_X86__)
#define E_CANNOT_DEBUG_AMD64 _HRESULT_TYPEDEF_(E_CANNOT_DEBUG_AMD64__)
#define E_CANNOT_DEBUG_ARM _HRESULT_TYPEDEF_(E_CANNOT_DEBUG_ARM__)
#define E_CANNOT_DEBUG_ARM64 _HRESULT_TYPEDEF_(E_CANNOT_DEBUG_ARM64__)
#define E_INVALID_ATTACH_ORDER _HRESULT_TYPEDEF_(E_INVALID_ATTACH_ORDER__)
#define E_ASYNC_OPERATION_RUNNING _HRESULT_TYPEDEF_(E_ASYNC_OPERATION_RUNNING__)
#define E_ASYNC_OPERATION_NOT_RUNNING _HRESULT_TYPEDEF_(E_ASYNC_OPERATION_NOT_RUNNING__)
#define E_OPERATION_IN_WORKLIST _HRESULT_TYPEDEF_(E_OPERATION_IN_WORKLIST__)
#define E_FAILED_TO_POST_COMPLETION _HRESULT_TYPEDEF_(E_FAILED_TO_POST_COMPLETION__)
#define E_CANCELLING_OPERATIONS _HRESULT_TYPEDEF_(E_CANCELLING_OPERATIONS__)
#define E_TTD_TRACE_ALREADY_LAUNCHED _HRESULT_TYPEDEF_(E_TTD_TRACE_ALREADY_LAUNCHED__)
#define E_SCRIPT_NOT_SUPPORTED _HRESULT_TYPEDEF_(E_SCRIPT_NOT_SUPPORTED__)
#define E_DUMP_WITH_MULTIPLE_APPDOMAINS _HRESULT_TYPEDEF_(E_DUMP_WITH_MULTIPLE_APPDOMAINS__)
#define E_VIRTUAL_THREAD _HRESULT_TYPEDEF_(E_VIRTUAL_THREAD__)
#define E_LIVE_TASKS_NOT_SUPPORTED_NETCORE_2_2 _HRESULT_TYPEDEF_(E_LIVE_TASKS_NOT_SUPPORTED_NETCORE_2_2__)
#define E_LIVE_TASKS_NOT_SUPPORTED_ATTACH _HRESULT_TYPEDEF_(E_LIVE_TASKS_NOT_SUPPORTED_ATTACH__)
#define E_DUMP_TASKS_NOT_SUPPORTED_CLR_VERSION _HRESULT_TYPEDEF_(E_DUMP_TASKS_NOT_SUPPORTED_CLR_VERSION__)
#define E_LIVE_TASKS_NOT_SUPPORTED_BEFORE_WINDOWS_8_1 _HRESULT_TYPEDEF_(E_LIVE_TASKS_NOT_SUPPORTED_BEFORE_WINDOWS_8_1__)
#define E_DOCKER_COMMAND_ERROR _HRESULT_TYPEDEF_(E_DOCKER_COMMAND_ERROR__)
#define E_DOCKER_MSVSMON_LAUNCH_FAILED _HRESULT_TYPEDEF_(E_DOCKER_MSVSMON_LAUNCH_FAILED__)
#define E_DOCKER_CONTAINER_NOT_RUNNING _HRESULT_TYPEDEF_(E_DOCKER_CONTAINER_NOT_RUNNING__)
#define E_DOCKER_MISSING_FILEORDIRECTORY _HRESULT_TYPEDEF_(E_DOCKER_MISSING_FILEORDIRECTORY__)
#define E_DOCKER_COMMAND_RAN_LONGER _HRESULT_TYPEDEF_(E_DOCKER_COMMAND_RAN_LONGER__)
#define E_DOCKER_NOT_FOUND _HRESULT_TYPEDEF_(E_DOCKER_NOT_FOUND__)
#define E_DOCKER_INVALIDARCHITECTURE _HRESULT_TYPEDEF_(E_DOCKER_INVALIDARCHITECTURE__)
#define E_DOCKER_UNABLETODOWNLOAD _HRESULT_TYPEDEF_(E_DOCKER_UNABLETODOWNLOAD__)
#define E_DOCKER_UNEXPECTEDOUTPUT _HRESULT_TYPEDEF_(E_DOCKER_UNEXPECTEDOUTPUT__)
#define E_DOCKER_NOT_REGISTERED_PROPERLY _HRESULT_TYPEDEF_(E_DOCKER_NOT_REGISTERED_PROPERLY__)
#define E_DOCKER_USERCANCELED _HRESULT_TYPEDEF_(E_DOCKER_USERCANCELED__)
#define E_DOCKER_MISSINGREQUIREDTOOL _HRESULT_TYPEDEF_(E_DOCKER_MISSINGREQUIREDTOOL__)
#define E_DOCKER_MATCHINGARCHSUPPORT _HRESULT_TYPEDEF_(E_DOCKER_MATCHINGARCHSUPPORT__)
#define E_DECOMPILED_SYMBOLS_MUST_BE_PORTABLE_PDB_FORMAT _HRESULT_TYPEDEF_(E_DECOMPILED_SYMBOLS_MUST_BE_PORTABLE_PDB_FORMAT__)
#define E_DECOMPILED_SYMBOLS_NO_EDIT_IN_CONTINUE _HRESULT_TYPEDEF_(E_DECOMPILED_SYMBOLS_NO_EDIT_IN_CONTINUE__)
#define E_DECOMPILED_SYMBOLS_SUPRESS_ILDASM _HRESULT_TYPEDEF_(E_DECOMPILED_SYMBOLS_SUPRESS_ILDASM__)
#define E_ENC_CANNOT_REMAP_AFTER_SET_NEXT_STATEMENT _HRESULT_TYPEDEF_(E_ENC_CANNOT_REMAP_AFTER_SET_NEXT_STATEMENT__)
#define E_BCL_METADATA_MISSING _HRESULT_TYPEDEF_(E_BCL_METADATA_MISSING__)
#define E_LIVE_TASKS_NOT_SUPPORTED_TOO_MANY_SESSIONS _HRESULT_TYPEDEF_(E_LIVE_TASKS_NOT_SUPPORTED_TOO_MANY_SESSIONS__)
#define E_DECOMPILED_UNSUPPORTED_MEMORY_LAYOUT _HRESULT_TYPEDEF_(E_DECOMPILED_UNSUPPORTED_MEMORY_LAYOUT__)
#define E_PROFILER_LAUNCH_FAILURE _HRESULT_TYPEDEF_(E_PROFILER_LAUNCH_FAILURE__)
#define E_EMULATED_INTEROP_NOT_SUPPORTED _HRESULT_TYPEDEF_(E_EMULATED_INTEROP_NOT_SUPPORTED__)
#define E_UNTRUSTED_LOCATION _HRESULT_TYPEDEF_(E_UNTRUSTED_LOCATION__)
#define E_NO_DEBUG_LAUNCH _HRESULT_TYPEDEF_(E_NO_DEBUG_LAUNCH__)
...
extern GUID guidReflectionEng;
extern GUID guidTraceReplayEng;
extern GUID guidAksSnasphot;
extern GUID guidMono;
...
extern GUID guidDockerWindowsPortSupplier ;
...
extern GUID guidSourceHashSHA256;
extern GUID guidSourceHashMD5Normalized;
extern GUID guidSourceHashSHA1Normalized;
extern GUID guidSourceHashSHA256Normalized;
