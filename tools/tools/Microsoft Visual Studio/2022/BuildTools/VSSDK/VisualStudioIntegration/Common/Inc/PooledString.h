...
{
private:
// see comments in LockHelper.h for why this is only for unmanaged C++
    ...
    static RecursiveMutex& GetPoolLock()
    {
        // Use a wrapped static for the lock so we will have deterministic,
        // on-demand initialization.  This is important for CloudBuild.
        static RecursiveMutex poolLock;
        return poolLock;
    }
#else
    #define GetPoolLock()
...
public:
    ...
    static size_t GetPoolMemUsage (size_t* pcbWouldBe = NULL)
    {
        ...
        typename PoolType::const_iterator it;
    ...
    static void DumpPool()
    {
        ...
        typename SortedPoolType::const_iterator it;
...
private:
    ...
    unsigned int DecrementRefCount()
    {
        ...
        if (newRefCount == 0)
        {
            typename PoolType::iterator it = GetPool().find(InnerString);
...
#pragma VS_OPTIMIZE_DEFAULT
    ...
    static PairType& LookupWithAddRef (const StringType& s)
    {
        ...
        std::pair<PoolType::iterator, bool> pr;
        {
             ...
             * mutual exclusion vis-a-vis GetPoolLock() in DecrementRefCount works.
        ...
        typename PoolType::iterator itEntry   = pr.first;
         ...
         * instruction after we drop GetPoolLock() and thus already be 
...
// See bug https://devdiv.visualstudio.com/DevDiv/_workitems/edit/2068417. We ONLY have these here if UNIT_TESTING is defined because building msenv.dll doesn't define that and we handle this decleration elsewhere, whereas unit tests do, so we just do it here
// as they don't require the same #pragma that the code in msenv (env\msenv\core\ToolboxFrameworkEvents.cpp) does.

// Toolbox\ToolBoxTests unit test builds env\msenv\core\ToolboxFrameworkEvents.cpp, which defines the below items
// ToolboxFrameworkEvent\ToolBoxTests unit test builds env\msenv\core\ToolboxFrameworkEvents.cpp, which defines the below items
// GelUtil unit test build env\gel\lib\GelUtil\shape.cpp, which defines the below items
#if !defined(TOOLBOX_TEST) && !defined(GELUTIL_TEST)
