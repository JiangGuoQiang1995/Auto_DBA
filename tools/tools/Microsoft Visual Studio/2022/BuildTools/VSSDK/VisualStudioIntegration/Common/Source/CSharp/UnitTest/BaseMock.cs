...
namespace Microsoft.VsSDK.UnitTestLibrary
{
    ...
    public abstract class BaseMock
    {
        ...
        protected BaseMock()
        {
            this.callbacks = new Dictionary<string, EventHandler<CallbackArgs>>();
            this.returnValues = new Dictionary<string, object[]>();
            this.data = new Dictionary<string, object>();
            this.callsCount = new Dictionary<string, int>();
        }
        ...
        public void AddMethodCallback(string methodName, EventHandler<CallbackArgs> callback)
        {
            ...
            if (null != callback)
            {
                this.callbacks.Add(methodName, callback);
            }
        }
        ...
        {
            if (this.callsCount.ContainsKey(name))
            {
                return this.callsCount[name];
            }
            return 0;
        }
        ...
        {
            var total = 0;
            foreach (var i in this.callsCount.Values)
            {
                total += i;
            }
            return total;
        ...
        {
            if (!string.IsNullOrEmpty(name) && this.callsCount.ContainsKey(name))
            {
                this.callsCount.Remove(name);
            }
        }
        #endregion
    }
