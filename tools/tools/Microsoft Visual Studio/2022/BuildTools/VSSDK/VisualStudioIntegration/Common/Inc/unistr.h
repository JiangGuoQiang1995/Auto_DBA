...
// VS7:32377  non-const variants, like CRT
inline PWSTR WINAPI FindCharInSet         (_In_opt_count_(cchBuffer) PWSTR pchBuffer, int cchBuffer, PCWSTR set) {return const_cast<PWSTR>(FindCharInSet (const_cast<PCWSTR>(pchBuffer), cchBuffer, set));}
inline PWSTR WINAPI FindCharInOrderedSet  (_In_count_(cchBuffer) PWSTR pchBuffer, int cchBuffer, PCWSTR set) {return const_cast<PWSTR>(FindCharInOrderedSet (const_cast<PCWSTR>(pchBuffer), cchBuffer, set));}
inline PWSTR WINAPI FindChar              (_In_opt_z_ PWSTR psz, WCHAR ch) {return const_cast<PWSTR>(FindChar (const_cast<PCWSTR>(psz), ch));}
inline PWSTR WINAPI FindCharN             (_In_opt_count_(cch) PWSTR pchBuffer, int cch, WCHAR ch) {return const_cast<PWSTR>(FindCharN (const_cast<PCWSTR>(pchBuffer), cch, ch));}
inline PWSTR WINAPI FindLastChar          (_In_z_ PWSTR psz, WCHAR ch) {return const_cast<PWSTR>(FindLastChar (const_cast<PCWSTR>(psz), ch));}
inline PSTR  WINAPI FindLastCharA         (_In_z_ PSTR  psz, CHAR  ch) {return const_cast<PSTR>(FindLastCharA (const_cast<PCSTR>(psz), ch));}
