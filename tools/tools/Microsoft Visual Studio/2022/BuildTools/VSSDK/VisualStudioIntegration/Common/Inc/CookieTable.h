        ...
        switch (state.lastCookieUsed)
        {
            ...
            default:
                ...
                else
                {
                    ...
                    {
                        if(++loopCount == MaxCookie)
                        {
                            VSFAIL ("Should have found an available cookie value, but we checked every value between MinCookie and MaxCookie and didn't.");
                            throw (std::bad_alloc());
                        }

                        state.lastCookieUsed = std::max(MinCookie, 
                                                        static_cast<cookie_type>(((state.lastCookieUsed + 1) % MaxCookie)));
                    }
                }
                break;
        }
...
public:
    ...
    Func for_each (Func func, bool fIncludeInvalidElements = false)
    {
        ...
        else
        {
            ...
            {
                /*
                 * instead of locking the table while invoking the function, lock just long enough
                 * to take a snapshot of it and iterate over the snapshot
                 */
                std::vector<std::pair<cookie_type, mapped_type>> snapshot;
                {
                    Lock lock(this);
                    std::vector<std::pair<cookie_type, mapped_type>> snapshotT(begin(), end());
                    snapshot.swap(snapshotT);
                }
...
private:
    ...
    void ensure_pending_mods()
    {
        ...
        if (!have_pending_mods())
            m_spPendingMods = std::make_unique<PendingMods>();
    }
...
private:
    ...
    map_type                            m_map;
    std::unique_ptr<PendingMods>        m_spPendingMods;    // used when locked and mods are pending
    size_type                           m_cLocks;
