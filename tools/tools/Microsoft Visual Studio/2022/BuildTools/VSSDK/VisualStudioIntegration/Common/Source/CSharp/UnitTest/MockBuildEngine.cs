    ...
    sealed public class MockBuildEngine : IBuildEngine4
    {
        ...
        public void LogErrorEvent(BuildErrorEventArgs eventArgs)
        {
            if (eventArgs.File != null && eventArgs.File.Length > 0)
            {
                ...
                this.log += String.Format(CultureInfo.InvariantCulture, "{0}({1},{2}): ", eventArgs.File, eventArgs.LineNumber, eventArgs.ColumnNumber);
            }

            if (this.logToConsole)
                Console.Write("ERROR " + eventArgs.Code + ": ");
            this.log += "ERROR " + eventArgs.Code + ": ";
            ++this.errors;

            if (this.logToConsole)
                Console.WriteLine(eventArgs.Message);
            this.log += eventArgs.Message;
            this.log += "\n";
        }
        ...
        public void LogWarningEvent(BuildWarningEventArgs eventArgs)
        {
            ...
            this.log += "WARNING " + eventArgs.Code + ": ";
            ++this.warnings;
        ...
        {
            var resource = getString(resourceName);
            var stringToSearchFor = String.Format(CultureInfo.CurrentCulture, resource, parameters);
            this.AssertLogDoesntContain(stringToSearchFor);
        }
        ...
        public void AssertLogContainsMessageFromResource(GetStringDelegate getString, string resourceName, params string[] parameters)
        {
            var resource = getString(resourceName);
            var stringToSearchFor = String.Format(CultureInfo.CurrentCulture, resource, parameters);
            this.AssertLogContains(stringToSearchFor);
        }
        ...
        {
            if (this.upperLog == null)
            {
                this.upperLog = this.log;
                this.upperLog = this.upperLog.ToUpperInvariant();
            }
              ...
              )
            {
                ...
                this.mockLogger.AssertLogContains(contains);
            }
        }
        ...
        public void AssertLogDoesntContain(string contains)
        {
            ...
            // MockLogger. Since MockLogger is also registered as
            // a logger it may have this string.
            this.mockLogger.AssertLogDoesntContain
            (
                contains
            );
        }
