...
public:
	...
	STDMETHOD(CountObjects)(DWORD dwFlags, ULONG *pCount)
	{
		...
		else if(dwFlags & GETOBJS_SELECTED)
		{
			...
			for(typename Derived_T::ItemsContainer::const_iterator item = rContainer.begin(); 
				item < rContainer.end();
				++item)
			{
	...
	STDMETHOD(GetObjects)(DWORD dwFlags, ULONG cObjects, IUnknown **pUnknown)
	{
		...
		if(dwFlags & GETOBJS_ALL)
		{
			...
			typename Derived_T::ItemsContainer::const_iterator item = rContainer.begin();
...
public:
		...
		_Out_ VSEDITORCREATEDOCWIN* pgrfCDW)
	{
		...
		*pgrfCDW = 0;
		...
		typename Derived_T::PhysicalViewId physicalViewID = 
