...
namespace Concurrency
{
    ...
    ///     <para>Programmatically modifying the process affinity after this method has been invoked will not cause the Resource Manager to re-evaluate
...
namespace details
{
    ...
    void ResourceManager::RetrieveSystemVersionInformation()
    {
        ...
        {
            // This implementation assumes that the target OS is Windows 7 or later.
    ...
    void ResourceManager::InitializeSystemInformation(bool fSaveTopologyInfo)
    {
...

        {
            ...
            unsigned int groupCountInNumaNodes = 0;
            unsigned int coreCount = 0;

            // Traverse the processor information buffer to find s_nodeCount and s_coreCount.
            // We will create one scheduling node per processor group in NUMA nodes.
            // (Note that the number of processor groups in NUMA nodes isn't necessarily
            // the same as the number of processor groups in processor packages.
            // Here, we use the processor groups in NUMA nodes because that
            // makes it easy to associate each group to a NUMA node number.)
                ...
                case RelationNumaNode:
                    {
                        ...
                        //   This field was introduced in Windows 20H2.
                        //   On earlier versions, this value is always 0.
                        const WORD rawGroupCount = pSysInfoEx->NumaNode.GroupCount;
                        const WORD totalGroupCount = rawGroupCount == 0 ? 1 : rawGroupCount;
                        for (WORD index = 0; index < totalGroupCount; ++index)
                        {
                            PGROUP_AFFINITY pGroupAffinity = &pSysInfoEx->NumaNode.GroupMasks[index];
                            ApplyAffinityRestrictions(pGroupAffinity);
                            if (pGroupAffinity->Mask != 0)
                            {
                                ++groupCountInNumaNodes;
                            }
                        }
                    }
                    break;
            ...
            ASSERT(groupCountInNumaNodes > 0);
            ASSERT(coreCount > 0);

            // These static variables should be updated exactly once, so that concurrent reads do not see intermediate values.
            s_nodeCount = groupCountInNumaNodes;
            ...
            while (byteOffset < s_logicalProcessorInformationLength)
            {
                if (pSysInfoEx->Relationship == RelationNumaNode)
                {
                    ...
                    for (WORD index = 0; index < totalGroupCount; ++index)
                    {
                        ...
                        {
#pragma warning(push)
#pragma warning(disable: 6385) // TRANSITION, VSO-1806041, avoid warning C6385: Reading invalid data from 'm_pGlobalNodes'.
                            // This index is valid due to ResourceManager::InitializeSystemInformation() above.
                            ASSERT(nodeIndex < m_nodeCount); // Below, we also assert that the final value of the index is equal to the count.
                            ...
                            m_pGlobalNodes[nodeIndex].Initialize(this, static_cast<USHORT>(nodeIndex), pGroupAffinity->Group, pGroupAffinity->Mask);
...
#pragma warning(pop)
    ...
    IExecutionResource * ResourceManager::RequestInitialVirtualProcessors(SchedulerProxy *pProxy, bool doSubscribeCurrentThread)
    {
        ...
        { // begin locked region
            ...
            // Increment this count before performing the allocation. If the new scheduler activates vprocs at the time
    ...
    ////        ReleaseOnlyBorrowedCores: only release borrowed cores.
    /// </summary>
    ...
    ExecutionResource * ResourceManager::PerformAllocation(SchedulerProxy *pSchedulerProxy, bool fInitialAllocation, bool fSubscribeCurrentThread)
    {
        ...
        if (fSubscribeCurrentThread)
        {
            ...
            else
            {
                ...
                // If we cannot find any cores at a use count of 0, the subscribed thread will either oversubscribe an existing core
...
#pragma warning(pop)
                    ...
                    for (unsigned int coreIndex = 0; coreIndex < pAllocatedNode->m_coreCount; ++coreIndex)
                    {
                        ...
                        if (pAllocatedCore->m_coreState == ProcessorCore::Allocated)
                        {
                            ...
                            if (pAllocatedCore->m_subscriptionLevel == 0)
                            {
                                ...
                                // If this core is idle, it cannot be taken away because it is fixed, and the fact that it contributes to min will take
    ...
    void ResourceManager::PopulateDynamicAllocationData()
    {
        ...
        for (pSchedulerProxy = m_schedulers.First(); pSchedulerProxy != NULL; pSchedulerProxy = m_schedulers.Next(pSchedulerProxy))
        {
            ...
            //  2) Have a suggested allocation greater than or equal to what they currently have.
    ...
    unsigned int ResourceManager::AdjustDynamicAllocation(unsigned int coresAvailable, unsigned int coresNeeded, unsigned int numReceivers)
    {
        ...
        {
            // We cannot satisfy the requirement with what we have available. We take the cores available and distribute them
                ...
                if (usedCoreQuota > 0)
                {
                    ...
                    for (unsigned int giv = 0; giv < numTotalGivers && foundUsedCores < usedCoreQuota; ++giv)
                    {
                        if (m_ppGivingProxies[giv]->m_pProxy->GetNumAllocatedCores() > m_ppGivingProxies[giv]->m_suggestedAllocation)
                        {
                            ...
                            if (pGivingNode->GetNumMigratableCores() > 0)
                            {
                                // Calculate the number of cores that actually correspond to the numbers of borrowed and owned cores we are allowed
                                                   ...
                                                   unsigned int numGivers)
    {
        ...
        if (totalCoresNeeded > 0)
        {
            ...
            ASSERT(numReceivers > 0);
            // Now for each receiver, try to satisfy cores on an unallocated node.
            do
            {
    ...
    void ResourceManager::DoCoreMigration()
    {
        ...
        // number of cores. After we have exhausted all such receivers, find fully loaded schedulers, and raise their suggested allocation to
        // their desired.
    ...
    void ResourceManager::DynamicResourceManager()
    {
        ...
        // 4) An activation or a deactivation occurred that could warrant core idle/busy notifications for existing schedulers. The state could
    ...
    ///
    ///     NOTE: We use the same mechanism the CLR uses for flushing write buffers in absence of the FlushProcessWriteBuffers API,
